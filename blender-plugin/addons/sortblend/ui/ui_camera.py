...
class SORTCameraData(bpy.types.PropertyGroup):
    lens_size: bpy.props.FloatProperty(
        name='Lens Size',
        default=0.0,
        min=0.0,
        description='Size of the camera lens for depth of field effect'
    )

    @classmethod
    def register(cls):
        bpy.types.Camera.sort_data = bpy.props.PointerProperty(
            name="SORT Data",
            type=cls,
            description="SORT camera settings"
        )
